...
M.get_example_sigmarc = function()
  local sigmarc_path = require("sigma.utils.config").custom.default_sigmarc_example_path

  -- store in data variable
  local data = utils.file("r", sigmarc_path)

  if not data then
    print "Error: Could not read exmaple sigmarc!"
    return false
  end
