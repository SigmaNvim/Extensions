...
M.beta_disclaimer = {
  {
    'IMPORTANT DISCLAIMER BEFORE USE:\n\nThe sigmaSnapshot feature is still in BETA. Use this feature with caution as it might still contain some bugs. Snapshots are stored in git branches. Do not manually switch between Snapshot branches, always use "sigmaSnapshotCheckout". Should your custom folder at any point "get lost", use the "git stash apply" command to restore it. sigma will always keep the last 4 versions of your custom folder in the Git Stash when creating new Snapshots or switching from the update branch to a Snapshot to ensure that you will not lose any data. If you encounter any bugs or have feature requests please create an issue in the main sigma repo and mention @LeonHeidelbach! If you like this feature, you can also let us know by commenting on the pinned issue for this BETA release! <3',
    "WarningMsg",
  },
}
...
M.cannot_update_snapshot = {
  {
    'Error: You cannot update while using a sigmaSnapshot! Use "sigmaSnapshotCheckout" to return to the update branch and try again.',
    "ErrorMsg",
  },
}
M.sigmarc_file_not_created = {
...
M.diverged_branches = {
  ...
  { "\n\nWould you like to reset sigma to the remote's HEAD? Local changes will be lost! " .. "[y/N]", "WarningMsg" },
}
...
M.modifications_detected_stash_restore_failed = {
  {
    "\nApplying stashed changes to the sigma "
...
M.reset_remote_head_success =
  { { "sigma's HEAD has successfully been reset to ", "Title" }, { "<UPDATE_BRANCH>" }, { ".\n\n", "Title" } }
...
M.update_confirm = { { "\nUpdate sigma? [y/N]", "WarningMsg" } }
M.update_continue = { { "\nWould you still like to continue with the update? [y/N]", "WarningMsg" } }
M.update_failed = { { "\nError: sigma Update failed.", "ErrorMsg" } }
M.update_failed_changes_restored =
  { { "Error: sigma Update failed.\n\n", "ErrorMsg" }, { "Local changes were restored." } }
M.update_success = { { "\nsigma succesfully updated.\n", "String" } }
M.wait_for_rollback_to_complete = {
  {
    'Packer will now start rolling back your custom plugins. This will take several seconds. Please wait for the completion notification and restart your editor. Do not run "PackerSync" while using a sigmaSnapshot!',
    "WarningMsg",
  },
}
