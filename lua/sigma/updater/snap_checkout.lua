local function snap_checkout()
  ...
  if selection == "c" then
    echo(misc.list_text_replace(prompts.cancelled_action, "<ACTION>", "sigmaSnapshotCheckout"))
    return
  end
  ...
  if current_branch_name == git.update_branch then
    -- make sure that sigmarc.lua exists, if not create it as a copy of the example
