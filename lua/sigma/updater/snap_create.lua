local function snap_create()
  ...
  local utils = require "sigma"
  local git = require "sigma.utils.git"
  local misc = require "sigma.utils.misc"
  local defaults = require "sigma.utils.config"
  local prompts = require "sigma.utils.prompts"
  ...
  -- @LeonHeidelbach on GitHub by creating an issue in the main sigma repo.
  ...
  if string.lower(name) == "c" or name == "" then
    echo(misc.list_text_replace(prompts.cancelled_action, "<ACTION>", "sigmaSnapshotCreate"))
    return
  end

  -- make sure that sigmarc.lua exists, if not create it as a copy of the example file
  local result = misc.ensure_file_exists(defaults.custom.default_sigmarc_path, misc.get_example_sigmarc())

  -- if the default custom config could not be created return
  if not result then
    echo(misc.list_text_replace(prompts.sigmarc_file_not_created, "<FILE_PATH>", defaults.custom.default_sigmarc_path))
    return false
  end
  ...
  -- create a temporary commit to add our changes in sigmarc.lua to the git history
