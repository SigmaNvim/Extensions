...
-- Custom theme picker
-- uses sigma_theme global variable
...
local function theme_switcher(opts)
  ...
  local local_utils = require "sigma"
  ...
  if next(themes) ~= nil then
    ...
    local current_theme = vim.g.sigma_theme
    local new_theme = ""
    local change = false
    ...
    picker.close_windows = function(status)
      ...
      if reload_theme(final_theme) then
        if change then
          ...
          if ans then
            ...
            -- will be used in restoring sigma theme var
            final_theme = current_theme
          end
        end
      else
      ...
      -- set sigma_theme global var
      vim.g.sigma_theme = final_theme
